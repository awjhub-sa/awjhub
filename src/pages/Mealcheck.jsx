...
export default function Mealcheck() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      let totalScore = 0;
      QUESTIONS.forEach(q => {
        if (answers[q.id] === 'نعم') totalScore += q.score;
      });
    ...
    <div dir="rtl" className="min-h-screen bg-[#FDFCFB] pb-32 font-arabic">
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-[#D1C4B9] z-50">
        <button onClick={handleSubmit} disabled={loading}
          className="w-full max-w-md mx-auto bg-[#A98159] text-white py-4 rounded-2xl font-bold text-lg shadow-lg flex items-center justify-center gap-3 active:scale-95 transition-all disabled:bg-gray-400">
          <Save size={20} />
