...
import {
  ...
  User, Building2, Clock, CheckCheck,
} from 'lucide-react';
...
const FILTERS = [
  ...
  ...SOURCES.map(s => ({ value: s.key, label: s.label, color: s.color })),
];
      ...
      <div className="bg-white rounded-2xl border border-[#EDE5DC] shadow-[0_2px_12px_rgba(45,41,38,0.07)] overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-base font-bold text-[#2D2926]">الإشعارات</h1>
          ...
          <button key={f.value} onClick={() => setFilter(f.value)}
            className={`px-4 py-2 rounded-2xl text-xs font-bold border transition-all ${
              filter === f.value
                ...
                : 'bg-white text-[#6D6E71] border-[#EDE5DC] hover:border-[#C9B8A8]'
            }`}
            style={filter === f.value
              ? { background: f.color
                  ? `linear-gradient(135deg, ${f.color}, ${f.color}CC)`
                  : 'linear-gradient(135deg, #2D2926, #4A3B35)' }
              : {}}>
            {f.label}
            <span className={`mr-1.5 ${filter === f.value ? 'opacity-70' : 'opacity-50'}`}>
              ({countFor(f.value)})
            </span>
          </button>
        ))}
        ...
        {loading ? (
          ...
          <div className="bg-white rounded-3xl border border-[#EDE5DC] py-20 text-center shadow-[0_2px_12px_rgba(45,41,38,0.06)]">
            <div className="w-14 h-14 rounded-2xl flex items-center justify-center mx-auto mb-3"
              ...
              <Bell size={24} className="text-[#C4A46E]" strokeWidth={1.5} />
            </div>
