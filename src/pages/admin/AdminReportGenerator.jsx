...
/* Arabic shaping for jsPDF now lives in a library — every report needs it,
   not only this one. See src/lib/arabicText.js. */
import { fixArabic, toArabicNum, wrapArabicLines } from '../../lib/arabicText.js';
...
/* `inline` drops the overlay chrome so the same builder can be a page rather
   than a dialog. One implementation either way — a second copy for the page
   would drift from this one on the first filter added. */
function ReportModal({ onClose, inline = false }) {
  ...
  return (
    <div
      className={inline ? 'w-full' : 'fixed inset-0 z-[200] flex items-center justify-center p-4'}
      dir="rtl"
    >
      {!inline && (
        <div
      ...
      {/* Panel */}
      <div
        className={`relative w-full bg-white rounded-3xl border border-[#E3E8EF] overflow-hidden ${
          inline
            ? 'max-w-2xl mx-auto shadow-[0_2px_16px_rgb(var(--c-ink)/0.08)]'
            : 'max-w-lg shadow-[0_24px_80px_rgba(30,27,46,0.25)]'
        }`}
        style={inline ? undefined : { animation: 'rg-slideUp 0.25s cubic-bezier(0.16,1,0.3,1) forwards' }}
      >
        {/* ── Modal header ── */}
        <div
        ...
        {/* ── Filters ── */}
        <div className={`p-6 space-y-5 ${inline ? '' : 'max-h-[58vh] overflow-y-auto'}`}>
...
/* The builder as a page, for the التقارير section in the sidebar. */
export function ReportBuilder() {
  return <ReportModal inline onClose={() => {}} />;
}
