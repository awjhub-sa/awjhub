...
/* Mina wears the brand navy; yes/no answers wear theirs. Nothing else on the
   page is allowed a colour. */
...
export default function SupMinaReadiness() {
  ...
  if (sweepMode && !sweepCenter) {
    return (
      <div dir="rtl" className="min-h-screen bg-canvas pb-28 font-arabic px-4 md:px-8">
        <TopBar title="جاهزية منى" onBack={() => navigate('/supervisor-home')} />
        ...
        <div className="max-w-3xl mx-auto space-y-4">
          <div className="rounded-[14px] border p-4 flex items-center gap-3.5"
            ...
            <IconTile Icon={MapPin} color={SITE} size="lg" />
            <div className="min-w-0">
              <p className="text-[10px] font-bold uppercase tracking-[0.16em]" style={{ color: SITE }}>جاهزية منى</p>
  ...
  if (!selectedTask) {
    return (
      <div dir="rtl" className="min-h-screen bg-canvas pb-28 font-arabic px-4 md:px-8">
        <TopBar title="جاهزية مشعر منى" onBack={() => navigate('/supervisor-home')} />
        ...
        <div className="max-w-2xl mx-auto mt-4">
          {tasksLoading ? (
            ...
            <div className="bg-white rounded-[14px] border border-line shadow-[0_1px_2px_rgb(var(--c-ink)/0.04)] text-center">
              <EmptyState Icon={Ban} title="لا توجد مهام حالياً" hint="لم يتم إسناد مهام جاهزية منى لهذا المركز بعد" />
                    ...
                    <button key={task.id}
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          <p className="font-bold text-ink text-[13.5px]">جاهزية مشعر منى</p>
    ...
    <div dir="rtl" className="min-h-screen bg-canvas pb-28 font-arabic px-4 md:px-8">
      ...
      <div className="relative rounded-[18px] overflow-hidden px-5 py-5 my-5"
        ...
        <div className="flex items-center gap-3.5 min-w-0">
          ...
          <div className="min-w-0">
            <h2 className="text-[19px] font-extrabold text-white leading-tight">{ALL_CRITERIA.length} بندًا للجاهزية</h2>
                          ...
                          {c.choices.map(choice => {
                            ...
                            return (
                              <button key={choice} onClick={() => handleAnswer(c.id, choice)}
                                className={`py-2.5 rounded-[11px] text-[12.5px] font-bold border transition-colors ${
      ...
      <div className="fixed inset-x-0 bottom-0 px-4 pt-3 pb-[max(0.75rem,env(safe-area-inset-bottom))] bg-white/95 backdrop-blur-md border-t border-line z-50">
        <button onClick={handleSubmit} disabled={loading}
          className="w-full max-w-md mx-auto min-h-[50px] bg-primary border border-primary text-white rounded-[12px] font-bold text-[15px] flex items-center justify-center gap-2 transition-colors disabled:opacity-60">
          {loading ? 'جاري الإرسال...' : <><Save size={18} weight="bold" /> حفظ وإرسال تقييم الجاهزية</>}
        </button>
      </div>
    </div>
  );
