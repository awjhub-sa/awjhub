...
import { ChevronRight, Save, CheckCircle2, AlertCircle, Home, ArrowLeft, Ban, Calendar, Camera, Loader2, X, Search, MapPin, Check } from 'lucide-react';
                ...
                return (
                  <button key={c.id}
                    ...
                    className={`group/ctr relative rounded-2xl p-3 text-right border-2 transition-all active:scale-[0.98] ${
                        ...
                        : 'bg-gradient-to-br from-white to-[#FDF8F0]/60 border-[#EDE5DC] hover:border-[#A98159] hover:shadow-[0_6px_18px_rgba(169,129,89,0.18)]'
                    }`}>
                    {done && (
                    ...
                    <div className="flex items-center gap-2.5">
                      <div className={`w-10 h-10 rounded-xl flex items-center justify-center shrink-0 border ${
                        done ? 'bg-green-50 border-green-200' : 'bg-[#FDF8F0] border-[#A98159]/20'
                      }`}>
                        <span className={`text-[12px] font-black tabular-nums ${done ? 'text-green-700' : 'text-[#A98159]'}`}>
                          {(c.id.match(/\d+/) || ['—'])[0]}
                        </span>
                      </div>
