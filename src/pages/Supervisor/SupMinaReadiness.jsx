      ...
      <div className="rounded-[2.5rem] p-6 my-6 text-white shadow-lg relative overflow-hidden" style={{ background: '#2D2926' }}>
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4 w-full">
          <div className="bg-white/5 rounded-2xl px-4 py-3 flex-1 min-w-[120px] flex flex-col items-center justify-center border border-white/10 shadow-sm">
            <span className="text-white/40 text-[10px] mb-1 font-medium">المشرف</span>
            <span className="text-white font-bold text-sm whitespace-nowrap">{profile?.nameAr || '—'}</span>
          </div>
          <div className="bg-white/5 rounded-2xl px-4 py-3 flex-1 min-w-[100px] flex flex-col items-center justify-center border border-white/10 shadow-sm">
            ...
            <span className="text-[#A98159] font-bold text-sm whitespace-nowrap">{selectedCenter}</span>
          </div>
          <div className="bg-white/5 rounded-2xl px-4 py-3 flex-1 min-w-[140px] flex flex-col items-center justify-center border border-white/10 shadow-sm">
            ...
            <span className="text-white font-bold text-sm whitespace-nowrap">{getCaterer(selectedCenter) || '—'}</span>
          </div>
        </div>
      </div>
                    ...
                    <div className="grid grid-cols-2 gap-2">
                      {c.choices.map(choice => (
                        <button key={choice} onClick={() => handleAnswer(c.id, choice)}
                          className={`py-3 rounded-2xl text-xs font-bold transition-all ${answers[c.id] === choice ? 'bg-[#A98159] text-white shadow-md' : 'bg-gray-50 text-[#6D6E71] border border-gray-100'}`}>
                          {choice}
                        </button>
                      ))}
                    </div>
                  ...
                  {(c.type === 'yesno' || c.type === 'yesno_detail') && (
                    <div className="flex gap-3">
                      <button onClick={() => handleAnswer(c.id, 'نعم')}
                        className={`flex-1 py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 ${answers[c.id] === 'نعم' ? 'bg-[#386B41] text-white shadow-lg' : 'bg-gray-50 border border-gray-100 text-[#6D6E71]'}`}>
                        <CheckCircle2 size={18} /> نعم
                      </button>
                      <button onClick={() => handleAnswer(c.id, 'لا')}
                        className={`flex-1 py-3.5 rounded-2xl font-bold flex items-center justify-center gap-2 ${answers[c.id] === 'لا' ? 'bg-[#BA1A1A] text-white shadow-lg' : 'bg-gray-50 border border-gray-100 text-[#6D6E71]'}`}>
                        <AlertCircle size={18} /> لا
                      </button>
                    </div>
                  )}
                  {c.type === 'yesno_detail' && answers[c.id] === 'نعم' && (
                    <input type="text" className="w-full mt-3 border rounded-xl px-4 py-3 text-sm focus:border-[#A98159] outline-none" value={details[c.id] || ''} onChange={e => handleDetail(c.id, e.target.value)} placeholder={c.detailLabel} />
                  )}
                </div>
              ))}
            </div>
