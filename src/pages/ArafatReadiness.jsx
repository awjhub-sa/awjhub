                ...
                <div key={c.id} className="bg-white rounded-3xl p-6 border border-[#D1C4B9] shadow-sm">
                  ...
                  {(c.type === 'yesno' || c.type === 'yesno_detail' || c.type === 'yesno_multi_detail') && (
                    <>
                      ...
                      {/* الحقل التقليدي (سؤال واحد) */}
                      {c.type === 'yesno_detail' && answers[c.id] === 'نعم' && (
                        <input type="text" value={details[c.id] || ''} onChange={e => handleDetail(c.id, e.target.value)}
                          className="w-full mt-3 border border-[#D1C4B9] rounded-xl px-4 py-3 text-sm focus:border-[#A98159] outline-none" placeholder={c.detailLabel} />
                      )}

                      {/* الحقلين المخصصين للسؤال 14 */}
