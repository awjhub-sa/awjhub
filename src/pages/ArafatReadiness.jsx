    ...
    criteria: [
      ...
      { id: 13, text: 'هل تم توفير فلتر للمياه (فلتر ثلاثي) داخل المطبخ؟', score: 0.57, type: 'yesno' },
      {
        ...
        fields: [
          ...
          { key: 'thermosType', label: 'ترامس وانواعها ( درفه أو درفتين )', type: 'text' }
        ]
      },
...
export default function ArafatReadiness() {
        ...
        <div className="max-w-2xl mx-auto mt-4">
          {tasksLoading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 gap-3 text-center">
              ...
              <p className="text-[#9D8F85] text-sm max-w-xs">لم يتم إسناد مهام جاهزية عرفة لمركزك بعد</p>
  ...
  return (
    <div dir="rtl" className="min-h-screen bg-[#FDFCFB] pb-28 font-arabic px-4 md:px-8">
        ...
        <div className="flex justify-between items-center mb-6">
          <div className="flex items-center gap-3">
            <div className="bg-white/10 p-3 rounded-2xl">
              <Mountain className="text-[#A98159]" size={28} />
            </div>
            <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {c.choices.map(choice => (
                      <button key={choice} onClick={() => handleAnswer(c.id, choice)}
                        className={`py-3 rounded-2xl text-sm font-bold transition-all ${answers[c.id] === choice ? 'bg-[#A98159] text-white shadow-md' : 'bg-gray-50 text-[#6D6E71] border border-gray-100'}`}>
                        {choice}
                      </button>
                    ))}
                  </div>
                  ...
                  <>
                    <div className="flex gap-3">
                      <button onClick={() => handleAnswer(c.id, 'نعم')}
                        className={`flex-1 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all ${answers[c.id] === 'نعم' ? 'bg-[#386B41] text-white shadow-lg' : 'bg-gray-50 text-[#6D6E71] border border-gray-100'}`}>
                        <CheckCircle2 size={18} /> نعم
                      </button>
                      <button onClick={() => handleAnswer(c.id, 'لا')}
                        className={`flex-1 py-3 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all ${answers[c.id] === 'لا' ? 'bg-[#BA1A1A] text-white shadow-lg' : 'bg-gray-50 text-[#6D6E71] border border-gray-100'}`}>
                    ...
                    {c.type === 'yesno_detail' && answers[c.id] === 'نعم' && (
                      <input type="text" value={details[c.id] || ''} onChange={e => handleDetail(c.id, e.target.value)}
                        className="w-full mt-3 border border-[#D1C4B9] rounded-xl px-4 py-3 text-sm focus:border-[#A98159] outline-none"
                      ...
                      <div className="grid grid-cols-1 gap-3 mt-3">
                        {c.fields.map(field => (
                          <input key={field.key} type={field.type}
                            ...
                            className="w-full border border-[#D1C4B9] rounded-xl px-4 py-3 text-sm focus:border-[#A98159] outline-none"
      ...
      <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-[#D1C4B9] z-[100]">
        <button onClick={handleSubmit} disabled={loading}
          className="w-full max-w-md mx-auto bg-[#A98159] text-white py-4 rounded-2xl font-bold text-lg shadow-lg flex items-center justify-center gap-2 disabled:opacity-50 transition-all">
